        ...
        for i in tqdm(range(1, 2)):
            ...
            for index in tqdm(range(len(response))):
                ...
                if movie['id'] not in movies:
                    movies.add(movie['id'])
                cursor.execute(
                    "INSERT INTO Movie (id, original_language, title, overview, release_date, poster_path, vote_average, genre_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (id) DO NOTHING;", (movie['id'], movie['original_language'], movie['title'], movie['overview'], movie['release_date'], movie['poster_path'], movie['vote_average'], id))
    conn.commit()
    conn.close()
    cursor.close()
    pass
